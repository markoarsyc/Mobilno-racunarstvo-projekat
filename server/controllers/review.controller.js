...
const reviewController = {
  ...
  // Ucitavanje recenzije po filmu i po korisniku
  async getReviewByUserAndMovie(req, res) {
    try {
      const { userId, movieAPI } = req.params;

      if (!userId || !movieAPI) {
        return res
          .status(400)
          .json({ message: "User ID and Movie Title are required." });
      }
