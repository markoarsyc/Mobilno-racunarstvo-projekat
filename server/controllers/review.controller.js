...
const reviewController = {
  ...
  async createReview(req, res) {
    try {
      ...
      const newReview = new Review({ user, movieAPI, movieTitle, rating, review });
      await newReview.save();

      res.status(201).json(newReview);
    } catch (error) {
      if (error.code === 11000) {
        res.status(400).json({ message: "User has already reviewed this movie." });
      } else {
  ...
  async updateReview(req, res) {
    try {
      ...
      if (rating == null && !review) {
        return res.status(400).json({ message: "At least one field (rating or review) must be provided." });
      }
